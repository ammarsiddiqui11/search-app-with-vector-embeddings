...
function MovieDetail() {
  ...
  useEffect(() => {
    axios.get(`${import.meta.env.BASE_URL}/recommend/${id}`)
      .then(res => setRecommendations(res.data.data))
      .catch(console.error);
  }, [id]);
  useEffect(() => {
    axios.get(`${import.meta.env.BASE_URL}/movies/${id}`)
      .then(res => setMovie(res.data.data))
      .catch(console.error);
  }, [id]);

//   useEffect(() => {
//     axios.get(`${import.meta.env.BASE_URL}/search?query=${id}`) // Simulate getting single movie detail
